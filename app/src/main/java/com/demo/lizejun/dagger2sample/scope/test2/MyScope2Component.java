...
//@Singleton
//(这也会报错)，我们不能给MainActivityComponent也标注@Singleton，并且dagger2也不允许。因为单例依赖单例是不符合设计原则的，我们需要自定义一个@Scope注解。
@MyActivityScope2
@Component(dependencies = DenpencyPotComponent2.class)
public interface MyScope2Component {
	void inject(MyScopeActivity2 activity);
	void inject2(MyScopeActivity2_1 activity);
}
